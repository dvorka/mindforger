#define MINDFORGER_VERSION_MAJOR "1"
#define MINDFORGER_VERSION_MINOR "49"
#define MINDFORGER_VERSION_REVISION "2"
#define MINDFORGER_VERSION_STRING "1.49.2"
#define MINDFORGER_VERSION_DWORD 1,49,0,2
#define MINDFORGER_APP_NAME   "MindForger"
#define MINDFORGER_APP_DESCIPTION  "MindForger Thinking Notebook"
#define MINDFORGER_APP_AUTHOR "Martin Dvorak"
#define MINDFORGER_APP_URL "https://www.mindforger.com"
#define MINDFORGER_APP_COMPANY   MINDFORGER_APP_NAME
#define MINDFORGER_APP_LEGAL "\xA9 2019 Martin Dvorak. All Rights Reserved"
#define MINDFORGER_APP_EXE "mindforger.exe" 
