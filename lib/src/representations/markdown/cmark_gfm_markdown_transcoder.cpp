    ...
    if(format == RepresentationType::HTML) {
        ...
        if(doc) {
            ...
            if (rendered_html) {
                ...
                rendered = regex_replace(rendered, url_regex, "<a class=\"mf-url\" href=\"$1\">$1</a>");

                // regex for links generated by the autolinking - AutolinkingPreprocessor
                // injects regular Markdown [text](url) links whose href always starts with
                // the mindforger://links.mindforger.com/ prefix, so by the time they
                // reach this transcoder they are indistinguishable from a manually
                // typed link - tag them with a dedicated class so they can be styled
                // like autolinks (blue) rather than manual links (green)
                regex autolink_regex("<a href=\"(mindforger://[^\"]+)\">");
                rendered = regex_replace(rendered, autolink_regex, "<a class=\"mf-autolink\" href=\"$1\">");
