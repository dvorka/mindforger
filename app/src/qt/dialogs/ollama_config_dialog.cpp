...
void OllamaConfigDialog::setEditProvider(const LlmProviderConfig& provider)
{
    editMode = true;
    providerConfig = provider;
    configValid = provider.isValid;

    setWindowTitle(tr("Edit ollama Provider"));
...
{
    if(editMode) {
        urlEdit->setText(QString::fromStdString(providerConfig.url));
        llmModelCombo->clear();
        llmModelCombo->addItem(QString::fromStdString(providerConfig.llmModel));
...
void OllamaConfigDialog::handleProbe()
{
    ...
    if (!config.validateOllamaProviderInput(url, model, errorMessage)) {
        QMessageBox::critical(
            this,
    ...
    if (wingman.didLastListModelsSucceed()) {
        QMessageBox::information(
            ...
            tr("ollama provider configuration is valid - successfully connected to the ollama server."));
        configValid = true;
    } else {
        QMessageBox::critical(
            this,
            tr("Configuration Invalid"),
            tr("Could not connect to the ollama server at %1. Please check the URL and try again.")
